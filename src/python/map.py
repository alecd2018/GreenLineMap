...
class Line(object):
    ...
    def jsonify(self):
        ...
        for s in self.stops.sortedStops:
            stop = self.stops.stops[s]
            data['stops'][stop.id] = {
                'loc' : [stop.location[0], stop.location[1]],
                'pixelLoc' : stop.pixelLocation,
                'name' : stop.name
            }
        for t in self.trains.trains:
            train = self.trains.trains[t]
...
class Train(object):
    ...
    def __init__(self, tID, location, direction, timestamp):
        self.id = tID
        self.location = location
...
class Trains(object):
    ...
    def update(self, name, stops):
        ...
        for train in resp:
            ...
            t = Train(tID, loc, train['attributes']['direction_id'], timestamp)
